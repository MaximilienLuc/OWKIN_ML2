...
\begin{titlepage}
    ...
    {\scshape\Large École Polytechnique \par}
    \vspace{2cm}
    
    \hrule height 1.5pt
    \vspace{1.5cm}
    {\huge\bfseries Detecting PIK3CA mutation in Breast Cancer from Histopathological Slides\par}
    \vspace{1cm}
    {\LARGE\itshape OWKIN Machine Learning Challenge\par}
    \vspace{1.5cm}
    \hrule height 1.5pt
    \vspace{3cm}
    
    {\Large\textbf{Nicolas Trancart} \\[-0.2em]
    \normalsize\texttt{nicolas.trancart@polytechnique.edu}\par}
    \vspace{1cm}
    
    {\Large\textbf{Maximilien Lucille} \\[-0.2em]
    \normalsize\texttt{maximilien.lucille@polytechnique.edu}\par}
    
    \vfill
    {\Large 1st March 2026\par}
\end{titlepage}
...
\noindent This report presents our approach to the OWKIN Machine Learning Challenge, a weakly supervised binary classification task aiming to predict \textit{PIK3CA} mutation status from histopathological whole-slide images represented as bags of pre-extracted MoCo features. 
The dataset comprises 344 training patients and 149 test patients originating from different hospital centers, creating a challenging setting characterized by small sample size, high dimensionality, and pronounced domain shift.

We systematically explored multiple aggregation strategies and modeling approaches, including mean and max pooling, extended statistical summaries, regularized linear models, gradient boosting, attention-based Multiple Instance Learning (MIL), anomaly detection, and ensemble methods. 
Our best public leaderboard performance (AUC = 0.6554, 55th rank) was achieved with a simple Optuna-tuned Random Forest applied to mean-pooled features.

Our experiments provide empirical evidence that the mutation signal is weak, semi-localized, and variance-dominated under domain shift.
The results suggest that the \textit{PIK3CA} mutation signal is weak and moderately diffuse, rather than being driven by a few highly discriminative tiles. 
Increasing model complexity or expanding feature dimensionality systematically improved cross-validation scores but degraded generalization performance, revealing variance dominance in this low-$n$, high-$d$ regime.

We further demonstrate that domain shift across hospital centers constitutes the primary bottleneck for generalization. 
Cross-validation strategies that fail to simulate this shift yield systematically optimistic estimates, and overly aggressive domain-invariant feature pruning may inadvertently remove genuine biological signal.

Overall, this study highlights that, under severe data scarcity and distribution shift, parsimonious models with implicit variance control can outperform more expressive methods. 
The findings emphasize the importance of understanding signal structure and validation design rather than relying solely on model complexity.
...
\subsection{Medical and Scientific Context}

Breast cancer is one of the most common cancers worldwide. 
Modern treatment strategies increasingly rely on molecular characteristics of the tumor, in addition to its visual appearance under the microscope.

One important molecular alteration is the \textit{PIK3CA} mutation. 
This mutation affects the PI3K signaling pathway, which regulates cell growth and survival. 
It is present in a significant proportion of breast cancer patients and may influence prognosis and response to targeted therapies.

In clinical practice, the presence of a \textit{PIK3CA} mutation is determined using genetic sequencing techniques. 
However, these techniques are costly and not always available. 
Being able to predict the mutation directly from routine histopathology images would therefore be valuable, especially in low-resource settings.

Whole-Slide Images (WSIs) are high-resolution digital scans of tumor tissue. 
They contain rich morphological information but are extremely large and complex. 
The challenge is to determine whether subtle visual patterns in these images are associated with the presence of a \textit{PIK3CA} mutation.

\subsection{Problem Formulation}

The task is formulated as a binary classification problem at the patient level. 
For each patient, the goal is to predict the presence (1) or absence (0) of a \textit{PIK3CA} mutation using histopathology data.

Each patient is represented by approximately 1,000 image tiles extracted from a Whole-Slide Image. 
However, labels are available only at the patient level. 
We do not know which specific regions of the tissue may carry the mutation-related signal.

This defines a weakly supervised learning setting. 
More precisely, the problem follows a Multiple Instance Learning (MIL) structure: each patient corresponds to a bag of tiles, and only the bag is labeled. 
The model must therefore learn how to aggregate tile-level information into a single patient-level prediction.

\subsection{Challenge-specific constraints}

Several structural characteristics make this task particularly difficult.

\paragraph{Small sample regime.}
The training set contains only $n = 344$ patients, while the test set contains $n = 149$ patients. 
Learning reliable patterns from a limited number of samples while avoiding overfitting is a central challenge.

\paragraph{High dimensionality.}
Each tile is represented by a 2048-dimensional feature vector extracted using a pre-trained neural network. 
As a result, each patient is described by a $1000 \times 2048$ feature matrix before aggregation. 
Even after summarizing tile-level features, the resulting representation remains high-dimensional relative to the number of patients.

\paragraph{Domain shift.}
The data originate from five hospital centers. 
The training set contains slides from centers $\{C1, C2, C5\}$, while the test set contains slides from centers $\{C3, C4\}$. 
Differences in staining protocols, scanners, and patient populations may introduce systematic variations. 
The model must therefore generalize to unseen institutions.

\paragraph{MIL structure.}
Because labels are assigned at the patient level and not at the tile level, only a subset of tiles may contain diagnostically relevant information. 
Designing an appropriate aggregation strategy is therefore critical.

\subsection{Evaluation Metric}

Model performance is evaluated using the Area Under the Receiver Operating Characteristic Curve (AUC-ROC).

The AUC measures the ability of the model to rank positive patients above negative patients, independently of any classification threshold. 
Formally,
...
\sum_{i \in \mathcal{P}}
\sum_{j \in \mathcal{N}}
\mathbb{1} \left[ \hat{y}_i > \hat{y}_j \right],
\]

where $\mathcal{P}$ and $\mathcal{N}$ denote the sets of positive and negative patients, and $\hat{y}_i$ is the predicted probability for patient $i$.

An AUC of 0.5 corresponds to random guessing, while an AUC of 1.0 indicates perfect ranking. 
This metric is appropriate in this setting because it is robust to class imbalance and focuses on ranking quality rather than absolute probability calibration.
...
\newpage
\section{Understanding the Nature of the Mutation Signal}
\label{sec:signal}

A central question of this project is not only how to predict the mutation, but what kind of signal the mutation induces in histopathological images. 
Understanding whether the \textit{PIK3CA} mutation manifests as a strong localized pattern or as a subtle global alteration is essential to design appropriate aggregation and modeling strategies.

\subsection{Localized versus Diffuse Signal Hypothesis}

We formulated two competing hypotheses regarding the spatial structure of the mutation signal.

\paragraph{Localized hypothesis.}
If the mutation generates highly discriminative morphological patterns confined to a small subset of tiles (``hotspots''), then max pooling or top-$k$ pooling should be effective. 
Under this assumption, only a few tiles would carry strong predictive information, and capturing the highest activation values would be sufficient for patient-level prediction.

\paragraph{Diffuse hypothesis.}
Alternatively, the mutation may induce subtle but widespread changes in tissue morphology. 
In this case, no single tile would be strongly discriminative, but the overall distribution of tile features would shift slightly between mutated and non-mutated patients. 
Mean pooling would then be more appropriate, as it captures global distributional differences.

Empirically, mean pooling consistently outperformed strict max pooling strategies, while extended statistics (e.g., skewness, kurtosis, high-order moments) did not yield stable improvements on the leaderboard. 
This suggests that the signal is not purely sparse and extreme, but neither is it uniformly distributed across all tiles.

\subsection{Evidence from Model Behavior}

The behavior of different models further informs our understanding of the signal structure.

First, increasing model complexity (gradient boosting, deep attention-based MIL) did not significantly improve performance over a moderately regularized Random Forest applied to mean-pooled features. 
If the mutation signal relied on highly non-linear interactions between tiles, more expressive models would be expected to provide substantial gains.

Second, expanding the feature space from 2048 dimensions (mean pooling) to 6144 or 18,432 dimensions (extended statistics) improved cross-validation scores but degraded leaderboard performance. 
This indicates that richer distributional summaries may capture center-specific artifacts rather than robust biological patterns.

Taken together, these observations suggest that the mutation signal is weak, moderately structured, and primarily encoded in global shifts of the tile feature distribution rather than in rare, extreme activations.

\subsection{Interpretation of Tile-Level Probability Distributions}

To further investigate this hypothesis, we analyzed tile-level probability distributions (see Section~6.3). 
Tiles from mutated patients exhibit a heavier right tail compared to non-mutated patients, indicating the presence of moderately high-probability regions (Figure~\ref{fig:tile_proba}).

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{tile_proba_distribution.png}
\caption{Average Tile-Level Probability Distributions per Patient Group. The mutated group shows a heavier right tail, indicating slightly elevated probabilities across subsets of tiles rather than a few highly discriminative regions.}
\label{fig:tile_proba}
\end{figure}

However, these high-probability tiles are not sufficiently dominant to allow pure max pooling to outperform mean aggregation. 
Instead, their contribution appears to shift the average tile-level prediction upward, explaining why mean pooling combined with a variance-reducing model such as Random Forest performs robustly.

Overall, the mutation signal appears to be semi-localized but weak: it manifests through a combination of slightly elevated probabilities across subsets of tiles rather than through singular, highly discriminative regions.

This characterization provides a unifying explanation for our empirical findings and guides the interpretation of subsequent modeling results.
...
Designing a reliable cross-validation (CV) procedure was one of the main methodological challenges of this competition. 
Because the training and test sets originate from different hospital centers, standard validation strategies may produce overly optimistic performance estimates.

\subsection{Why Standard Cross-Validation Overestimates Performance}

We first employed StratifiedKFold (k = 5), which preserves class balance across folds. 
However, this strategy mixes samples from the same hospital centers across training and validation splits. 
The model is therefore evaluated on data drawn from the same domain distribution it was trained on.

In the presence of center-specific artifacts (e.g., staining intensity, scanner characteristics), the model may learn domain signatures rather than purely biological features. 
This domain leakage leads to systematically optimistic AUC estimates and explains the consistent gap observed between cross-validation scores and public leaderboard performance.

\subsection{GroupKFold and High Variance under Limited Domains}

To better simulate the test scenario, we implemented GroupKFold using hospital center ID as the grouping variable. 
This ensures that entire centers are held out during validation, approximating an out-of-domain evaluation.

However, the training data contain only three centers (\{C1, C2, C5\}). 
GroupKFold therefore operates over only three groups, producing at most three effective validation splits. 
With such few domains, performance estimates exhibit high variance and depend strongly on which center is held out.

\subsection{Limitations of Leave-One-Center-Out (LOCO)}

A Leave-One-Center-Out (LOCO) strategy would most faithfully reproduce the test setting. 
Yet, with only three training centers, LOCO also yields three folds, resulting in unstable AUC estimates and unreliable model selection due to small validation sample sizes.

\subsection{Practical Compromise}

Given these constraints, we adopted patient-level StratifiedKFold with multiple repeats as our primary model selection strategy. 
Although this approach does not fully capture domain shift, it provides lower-variance estimates and more stable hyperparameter optimization.

Overall, this competition highlights a fundamental bias–variance tradeoff in validation under domain shift: domain-aware strategies reduce bias but increase variance when the number of domains is small, whereas standard CV provides stability at the cost of optimistic estimates.
...
The project can be interpreted as a structured exploration across three axes:
(i) aggregation hypothesis,
(ii) modeling hypothesis,
(iii) domain generalization strategy.
...
\vspace{0.5em}
\centering
{\large \textbf{Pipeline Architecture}}\\[1em]
\begin{flushleft}
\sffamily
\hspace*{3em}\textbf{MoCo Features} (1000 $\times$ 2048)\\[0.4em]
\hspace*{8em}$\downarrow$\\[0.4em]
\hspace*{3em}\textbf{Aggregation Hypothesis}\\
\hspace*{4em}$\vdash$ Mean (Global signal)\\
\hspace*{4em}$\lfloor$ Max / Top-$k$ (Localized signal)\\[0.4em]
\hspace*{8em}$\downarrow$\\[0.4em]
\hspace*{3em}\textbf{Modeling Hypothesis}\\
\hspace*{4em}$\vdash$ Linear boundary\\
\hspace*{4em}$\vdash$ Non-linear trees\\
\hspace*{4em}$\lfloor$ Attention MIL\\[0.4em]
\hspace*{8em}$\downarrow$\\[0.4em]
\hspace*{3em}\textbf{Domain Generalization Strategy}\\
\hspace*{4em}$\vdash$ Stratified CV\\
\hspace*{4em}$\vdash$ GroupKFold\\
\hspace*{4em}$\lfloor$ Drift removal\\[0.4em]
\hspace*{8em}$\downarrow$\\[0.4em]
\hspace*{3em}\textbf{Final Ensemble}
\end{flushleft}
\vspace{0.2em}
}}
\caption{High-level pipeline architecture. We systematically explored different hypotheses across aggregation, modeling, and domain generalization strategies before combining them into a final ensemble.}
\label{fig:pipeline}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{xgboost_feature_importance.png}
\caption{XGBoost Feature Importance (Weight), showing the relative contribution of the top features to the model.}
\label{fig:xgb_importance}
\end{figure}\subsection{Iteration 3: PCA + regularized linear models}
...
\subsection{Evidence for a Structural Performance Ceiling}

Across all experiments, model performance consistently converged within a narrow range (AUC $\approx$ 0.63–0.66 on the public leaderboard). 
Despite exploring increasingly complex approaches — gradient boosting, extended statistical features (up to 18,432 dimensions), attention-based MIL, anomaly detection, and ensemble strategies — no method produced a substantial and stable improvement over the mean-pooled Random Forest baseline.

Notably, boosting methods failed to outperform bagging-based Random Forests, and deep learning architectures did not yield clear gains despite their higher representational capacity. 
Similarly, expanding the feature space with higher-order statistics improved cross-validation scores but degraded generalization performance.

This convergence suggests the presence of a structural performance ceiling inherent to the task. 
Several factors may contribute to this limitation: (i) the MoCo embeddings may already compress most discriminative information, limiting downstream gains; (ii) the morphological signature of the \textit{PIK3CA} mutation may be intrinsically weak; and (iii) residual domain shift between hospital centers may introduce irreducible uncertainty.

Taken together, these observations indicate that model complexity is not the primary bottleneck. 
Rather, the limiting factor appears to be the strength and stability of the underlying biological signal under domain shift.
